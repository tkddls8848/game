...
namespace Detective.Tests
{
    ...
    public class TimelineBoardTests
    {
        ...
        public void EmptyState_KnowsNothing()
        {
            ...
            Assert.IsFalse(board.PlacementOf(Get(state.Database, "culprit"), 3).Known, "실제 스케줄을 흘리면 안 된다");
        }
        ...
        public void Alibi_RecordsClaimsForEveryTick()
        {
            ...
            {
                List<TimelineRecord> records = board.RecordsFor("culprit", t);
                Assert.AreEqual(1, records.Count, "t=" + t);
                Assert.AreEqual(RecordKind.Testimony, records[0].Kind);
            }
            Assert.AreEqual("a", board.RecordsFor("culprit", 3)[0].RoomId, "18:30엔 A방에 있었다고 주장한다(거짓)");
        }
        ...
        public void Sighting_ContradictsTestimony_AndBothAreKept()
        {
            ...
            List<TimelineRecord> at1810 = board.RecordsFor("culprit", 1);
            ...
            NpcPlacement placement = board.PlacementOf(Get(state.Database, "culprit"), 1);
        ...
        public void Evidence_RevealsWhereabouts_AndBeatsEverything()
        {
            ...
            List<TimelineRecord> records = board.RecordsFor("witness", 3);
            Assert.IsTrue(records.Exists(r => r.Kind == RecordKind.Evidence && r.SourceId == "ev_log"));
            Assert.AreEqual("물증: 기록부", board.PlacementOf(Get(state.Database, "witness"), 3).Caption);
        }
        ...
        public void ConfessionClaim_AddsSecondTestimony()
        {
            ...
            List<TimelineRecord> records = board.RecordsFor("culprit", 3);
            Assert.AreEqual(2, records.Count);
            Assert.AreEqual("study", board.PlacementOf(Get(state.Database, "culprit"), 3).RoomId, "나중에 한 말이 우선");
        }
